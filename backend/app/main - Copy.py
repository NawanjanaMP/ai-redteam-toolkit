...
from enums import AttackType, SeverityLevel
from schemas import TestReport as TestReportSchema, AttackResult as AttackResultSchema, TestReportCreate
...
class RedTeamTester:
    ...
    async def run_test(self, db: AsyncSession, request: TestRequest) -> TestReportSchema:
        ...
        report_data = schemas.TestReportCreate(
            test_id=test_id,
        ...
        db_report = await crud.create_test_report(db=db, report=report_data)
...
@app.post("/api/v1/test", response_model=schemas.TestReport)
async def run_security_test(request: TestRequest, db: AsyncSession = Depends(get_db)):
    try:
        report = await tester.run_test(db, request)
        return report
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Testing failed: {str(e)}")

@app.get("/api/v1/test/{test_id}", response_model=schemas.TestReport)
async def get_test_report(test_id: str, db: AsyncSession = Depends(get_db)):
    db_report = await crud.get_test_report(db, test_id=test_id)
...
async def get_statistics(db: AsyncSession = Depends(get_db)):
    reports = await crud.get_all_test_reports(db)
